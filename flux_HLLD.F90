...
#define SWR(A, B)  (0.5d0 - 0.5d0*sign(1.d0, (A))*sign(1.d0, (B)))
  ...
  subroutine flux( ql, qr, f, ndir)
    ...
    real(kind=DBL_KIND) :: &
         ...
         denom_r, &
                                ! 
         gm1i,sqrtpi4i,bxm,psim,sm,pta,sqlri,vm,wm,bym,bzm,gzm,signbxm,bxm2, &
...
#ifdef WO_DIVB_CLEANING
             f(i,j,k,MBX) = 0.
             f(i,j,k,MDB) = 0.
#endif !WO_DIVB_CLEANING
             bxm2 = bxm**2
             ! --------------------
             ! U_L; left variables
             ! --------------------
